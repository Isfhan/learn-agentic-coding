...
> **⏱️ Time:** ~2 hours · **Prereq:** Step 13 · **Next:** Step 14 · **Best for:** AI Engineer, FDE, AI PM (core)

> *"The spec is the new source of truth. Code is the build artifact."* — a useful 2026 shorthand, not a universal law
...
- What **Spec-Driven Development (SDD)** is and why it became a major 2026 agentic-coding pattern.
- The common 3-phase workflow: **Specify → Plan → Implement**.
- **GitHub Spec Kit** — a widely used, easy-to-start open-source SDD toolkit.
...
GitHub Spec Kit, AWS Kiro, OpenSpec, and Martin Fowler's SDD writeups all point in the same direction: **the bottleneck is shifting from writing code to knowing what to build and how to verify it.**
...
## 4. Two useful tools: GitHub Spec Kit and OpenSpec

Both are open source and tool-agnostic. Pick whichever fits your situation (table at the end of this section), and check their docs before using exact commands because agent integrations change quickly.

### 4A. GitHub Spec Kit (greenfield-friendly, phase-gated)

**Why Spec Kit:** open-source, free, and designed to work across many coding agents, including Cursor, Claude Code, GitHub Copilot, Gemini CLI/Antigravity, and Codex. It is a structured, phase-gated SDD toolkit.

Repo: `https://github.com/github/spec-kit` · License: MIT · Install: `uv tool install specify-cli`
...
**Why OpenSpec:** designed to be **fluid, iterative, and brownfield-first** — useful for adding SDD to an existing repo without rewriting everything. It supports many agents via slash commands.
...
**Core slash commands** (Claude Code uses `/opsx:<cmd>`; some tools may expose command names differently)
| Command | What it does |
|---------|--------------|
...
Expanded OpenSpec profiles can also expose commands such as `/opsx:verify`, `/opsx:sync`, and `/opsx:onboard`. Treat those as profile-dependent commands, not guaranteed defaults.
...
5. If your OpenSpec profile includes `/opsx:verify`, run it to check behavior against the spec.
...
| Python shop / already using `uv` | Spec Kit |
...
1. Install GitHub Spec Kit: `uv tool install specify-cli && specify init commit-coach`.
...
2. If your profile includes `/opsx:onboard`, run it for a guided tour. Otherwise, continue with `/opsx:propose`.
...
8. Run `/opsx:verify` if available, or manually compare the implementation against the proposal, spec delta, design, and tasks.
...
- Search YouTube for GitHub Spec Kit and OpenSpec walkthroughs from the last 6 months.
- Search conference talks for "spec-driven development AI coding" and compare how different teams use the term.
...
- David Lapsley, *Spec-Driven LLM Development (SDLD)* — https://sdd.sh/2026/01/spec-driven-llm-development/
